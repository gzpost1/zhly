...
public class AppWorkInfoService extends ServiceImpl<WorkInfoMapper, WorkInfoEntity> {
    ...
    public void checkWorkOrder(String processInstanceId) {
        if (Objects.nonNull(processInstanceId)) {
            ...
            //当前企业查看或者发起人是自己或者代录两者都可见
            boolean selectFlag = Objects.equals(workInfo.getCompanyId(), LoginInfoHolder.getCurrentOrgId()) ||
                    Objects.equals(workInfo.getActualUserId(), LoginInfoHolder.getCurrentUserId()) ||
                    Objects.equals(workInfo.getCustomerId(), LoginInfoHolder.getCurrentUserId());
            AssertUtil.isTrue(selectFlag, ResultCode.NO_OPERATION_PERMISSION.getMessage());
        }
    }
