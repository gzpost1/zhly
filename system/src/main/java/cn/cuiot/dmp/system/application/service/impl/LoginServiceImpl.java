...
import cn.cuiot.dmp.system.application.enums.DeletedFlagEnum;
...
public class LoginServiceImpl implements LoginService {
    ...
    public User authDmp(LoginReqDTO loginReqDTO) {
        ...
        // 账号存在
        String loginFailedUsersRedisKey = CacheConst.LOGIN_FAILED_USERS_REDIS_KEY + userEntity.getUserId();
        // 登录失败
    ...
    private void clearLoginFailedCount(String loginFailedUsersRedisKey) {
        ...
        Integer failedCounts = Optional.ofNullable(failedCountsStr).map(fcs -> Integer.parseInt(fcs)).orElse(0);
        // 登录失败次数未到上限
    ...
    private void recordLoginFailedCount(String userAccount) {
        String loginFailedNonExistUsersRedisKey = CacheConst.LOGIN_FAILED_NON_EXIST_USERS_REDIS_KEY + userAccount;
        // 登录失败次数自增
        long failedCounts = stringRedisTemplate.opsForValue().increment(loginFailedNonExistUsersRedisKey);
        // 设置过期时间
    ...
    public LoginResDTO loginIdentity(User validateUser, HttpServletRequest request) {
        ...
        if (pkUserId != null) {
            ...
            if (Objects.equals(validateUser.getLongTimeLogin(), UserLongTimeLoginEnum.OPEN.getCode())) {
                redisUtil.set(CacheConst.LOGIN_USERS_REFRESH_CODE + jwt, refreshCode, SecurityConst.REFRESH_SESSION_TIME * 18);
                // 登陆要求不互踢
                redisUtil.set(CacheConst.LOGIN_USERS_JWT + jwt, String.valueOf(pkUserId), Const.USER_LONG_TIME_LOGIN_SESSION_TIME);
                // 设置缓存，表示该用户为长期登录，方便gateway识别是否为长期登录用户，从而token续期
                redisUtil.set(CacheConst.USER_LONG_TIME_LOGIN + pkUserId, Const.STR_1, Const.USER_LONG_TIME_LOGIN_SESSION_TIME);
            } else {
                redisUtil.set(CacheConst.LOGIN_USERS_REFRESH_CODE + jwt, refreshCode, SecurityConst.REFRESH_SESSION_TIME);
                // 登陆要求不互踢
            ...
            for (String orgInfo : orgInfoList) {
                if (orgInfo.equals(pkOrgId)) {
                    redisUtil.lRemove(CacheConst.USER_ORG_CURRENT_KEY + pkUserId, SecurityConst.USER_ORG_LOGIN_RECORD_MAX, orgInfo);
                }
            }
            // 限制redis存储长度
    ...
    private static void checkOrgStatus(User validateUser, OrganizationResDTO organization) {
        // 查看账户状态
        if (organization == null || organization.getStatus() == null || OrgStatusEnum.DISABLE.getCode().equals(organization.getStatus())) {
    ...
    @Override
    public void logoutIdentity(HttpServletRequest request, String orgId, String userId, String userName) {
        boolean isSuccess = false;
        // 填充日志数据
        OperateLogDto platformOperateLogDTO = new OperateLogDto();
     ...
     */
    public void saveLog2Db(boolean isSuccess, OperateLogDto platformOperateLogDTO, HttpServletRequest request) {
        ...
        if (isSuccess) {
            // 操作成功
            platformOperateLogDTO.setStatusCode(cn.cuiot.dmp.common.enums.StatusCodeEnum.SUCCESS.getCode());
            platformOperateLogDTO.setStatusMsg(cn.cuiot.dmp.common.enums.StatusCodeEnum.SUCCESS.getName());
            platformOperateLogDTO.setLogLevel(cn.cuiot.dmp.common.enums.LogLevelEnum.INFO.getCode());
        } else {
            platformOperateLogDTO.setLogLevel(cn.cuiot.dmp.common.enums.LogLevelEnum.ERROR.getCode());
            platformOperateLogDTO.setStatusMsg(cn.cuiot.dmp.common.enums.StatusCodeEnum.FAILED.getName());
            platformOperateLogDTO.setStatusCode(cn.cuiot.dmp.common.enums.StatusCodeEnum.FAILED.getCode());
        }
