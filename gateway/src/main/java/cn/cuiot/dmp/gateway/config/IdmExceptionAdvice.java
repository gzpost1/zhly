...
public class IdmExceptionAdvice {
    ...
    public Object defaultErrorHandler(HttpMessageNotReadableException e) {
        ...
        if (rootCause instanceof BusinessException) {
            ...
            return new IdmResDTO<>(businessException.getErrorCode(),
                    businessException.getMessage() + businessException.getErrorMessage());
        }
        return new IdmResDTO<>(ResultCode.INVALID_PARAM_TYPE);
    }

    /**
     * BindException: org.springframework.validation.BeanPropertyBindingResult:异常处理 fromData参数校验
     */
