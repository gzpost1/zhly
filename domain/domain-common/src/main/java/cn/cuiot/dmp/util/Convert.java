...
public class Convert {
	...
	private static final int zeroX = 0x01;
	...
	public static byte[] byteConvert32Bytes(BigInteger n) {
		...
		if(n.toByteArray().length == NUMBER_33) {
			tmpd = new byte[32];
			System.arraycopy(n.toByteArray(), 1, tmpd, 0, 32);
		}
		else if(n.toByteArray().length == NUMBER_32) {
			tmpd = n.toByteArray();
		}
		else {
			tmpd = new byte[32];
			for(int i = 0; i < NUMBER_32 - n.toByteArray().length; i++) {
				tmpd[i] = 0;
			}
