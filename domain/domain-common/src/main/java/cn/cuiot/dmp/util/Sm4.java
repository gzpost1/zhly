 ...
 */
public class Sm4 {
	private static byte[] key = "JNl49EoDK1sayM6j".getBytes();
	...
	public static byte[] encodeSMS4(String plaintext, byte[] key)
		...
		return Sm4.encodeSMS4(encryptDES(plaintext).getBytes(), key);
	}
	...
	private void SMS4Crypt(byte[] Input, byte[] Output, int[] rk) {
		...
		for (r = 0; r < NUMBER_32; r += NUMBER_4) {
		...
		// Reverse
		for (int j = 0; j < BLOCK; j += NUMBER_4) {
	...
	public static byte[] hex2byte(byte[] b) {
		if ((b.length % NUMBER_2) != 0) {
			throw new IllegalArgumentException("长度不是偶数");
		}
		byte[] b2 = new byte[b.length / 2];
		for (int n = 0; n < b.length; n += NUMBER_2) {
	...
	public static String encryption(String encodingStr) {
		try {
			byte[] enOut = Sm4.encodeSMS4(encodingStr, key);
	...
	public static String decrypt(String Redaction) {
		try {
			...
			String deOutStr = Sm4.decodeSMS4toString(bytes, key);
	...
	public static void main(String[] args) {
		String encryption = Sm4.encryption("15898234567");
		System.out.println(encryption);
		String decrypt = Sm4.decrypt("B4DE23B10560DED029ABFF86E74860C18B403C6670967418CCCE630B404E778C");
