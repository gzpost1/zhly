...
public class GwWaterLeachAlarmTaskHandle {
                    ...
                    records.forEach(item -> {
                        try {
                            if (Objects.nonNull(item.getCompanyId()) && map.containsKey(item.getCompanyId())) {
                                ...
                                // 更新设备属性
                                GwWaterLeachAlarmPropertyEntity propertyEntity = respToProperty(resp);
                                if (Objects.nonNull(propertyEntity)) {
                                    propertyEntity.setGwWaterLeachAlarmId(item.getId());
     ...
     * 第三方响应转dto
     *
     * @return GwWaterLeachAlarmPropertySyncDto
     * @Param resp 参数
     */
    private GwWaterLeachAlarmPropertyEntity respToProperty(DmpDevicePropertyListResp resp) {
        if (Objects.isNull(resp) || CollectionUtils.isEmpty(resp.getList())) {
            return null;
        }

        GwWaterLeachAlarmPropertyEntity entity = new GwWaterLeachAlarmPropertyEntity();
        resp.getList().forEach(item -> {
            String key = item.getKey();
            Object value = item.getValue();

            if (GwWaterLeachAlarmPropertyEntity.BATTERY_LEVEL.equals(key) && Objects.nonNull(value)) {
                entity.setBatteryLevel(Integer.parseInt(value + ""));
            } else if (GwWaterLeachAlarmPropertyEntity.WATER_LEACH_STATE.equals(key)) {
                entity.setWaterLeachState(value + "");
            } else if (GwWaterLeachAlarmPropertyEntity.POWER_SAVING_MODE.equals(key)) {
                entity.setPowerSavingMode(value + "");
            } else if (GwWaterLeachAlarmPropertyEntity.CSQ.equals(key)) {
                entity.setCsq(value + "");
            }
        });
        return entity;
