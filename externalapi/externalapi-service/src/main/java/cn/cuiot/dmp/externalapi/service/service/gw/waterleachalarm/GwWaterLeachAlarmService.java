...
public class GwWaterLeachAlarmService extends ServiceImpl<GwWaterLeachAlarmMapper, GwWaterLeachAlarmEntity> {
     ...
     * @Param dto 参数
     */
    public void create(WaterLeachAlarmCreateDto dto) {
        // 数据校验
        checkDate(dto, null);
        // 企业id
        Long companyId = LoginInfoHolder.getCurrentOrgId();
     ...
     * @Param dto 参数
     */
    public void update(WaterLeachAlarmUpdateDto dto) {
    ...
    public GwWaterLeachAlarmDetailVO queryForDetail(Long id) {
        ...
        if (Objects.nonNull(propertyEntity)) {
            vo.setPowerSavingMode(GwWaterLeachAlarmPropertyEnums.powerSavingModeEnums.getNameByCode(propertyEntity.getPowerSavingMode()));
        }
        return vo;
    }
     ...
     * @Param id 数据id
     */
    public GwWaterLeachAlarmPropertyVO getProperty(Long id) {
        GwWaterLeachAlarmPropertyVO vo = new GwWaterLeachAlarmPropertyVO();

        GwWaterLeachAlarmPropertyEntity propertyEntity = getPropertyEntity(id);
        if (Objects.nonNull(propertyEntity)) {
            BeanUtils.copyProperties(propertyEntity, vo);
            // 设值电池电量名称
            vo.setBatteryLevelName(Objects.nonNull(vo.getBatteryLevel()) ? vo.getBatteryLevel() + "%" : null);
            // 设值水浸状态名称
            vo.setWaterLeachStateName(GwWaterLeachAlarmPropertyEnums.powerSavingModeEnums.getNameByCode(vo.getPowerSavingMode()));
            // 设值省电模式名称
            vo.setPowerSavingModeName(GwWaterLeachAlarmPropertyEnums.powerSavingModeEnums.getNameByCode(vo.getPowerSavingModeName()));
        }
        return vo;
    }
    ...
    private void handleSuccess(List<DmpDeviceBatchPropertyResp.SuccessDataItem> successList, Long companyId, String powerSavingMode) {
        ...
        if (CollectionUtils.isNotEmpty(successIotIds)) {
            List<GwWaterLeachAlarmEntity> updatedEntities = list(new LambdaQueryWrapper<GwWaterLeachAlarmEntity>()
                    .eq(GwWaterLeachAlarmEntity::getCompanyId, companyId)
                    .in(GwWaterLeachAlarmEntity::getIotId, successIotIds));

            // 批量更新设备的省电模式
            if (CollectionUtils.isNotEmpty(updatedEntities)) {
                List<Long> ids = updatedEntities.stream()
                        .map(GwWaterLeachAlarmEntity::getId)
                        .collect(Collectors.toList());
                LambdaUpdateWrapper<GwWaterLeachAlarmPropertyEntity> wrapper = new LambdaUpdateWrapper<>();
                wrapper.set(GwWaterLeachAlarmPropertyEntity::getPowerSavingMode, powerSavingMode);
                wrapper.in(GwWaterLeachAlarmPropertyEntity::getGwWaterLeachAlarmId, ids);
                gwWaterLeachAlarmPropertyMapper.update(null, wrapper);
            }
        }
    }
    ...
    private GwWaterLeachAlarmPropertyEntity getPropertyEntity(Long id) {
        return gwWaterLeachAlarmPropertyMapper.selectOne(
                new LambdaQueryWrapper<GwWaterLeachAlarmPropertyEntity>()
                        .eq(GwWaterLeachAlarmPropertyEntity::getGwWaterLeachAlarmId, id).last(" LIMIT 1 "));
    }
     ...
     * @Param id 数据id
     */
    private void checkDate(WaterLeachAlarmCreateDto dto, Long id) {
